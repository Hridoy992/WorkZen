export const AsyncReducer = (builder, thunk) => {
    builder
        ...
        .addCase(thunk.rejected, (state, action) => {
            if (action.payload && action.payload.gologin) {
                state.isLoading = false;
                state.error.status = false;
                state.error.message = action.payload.message
                state.error.content = action.payload
            }
            else if (action.payload && (action.payload.status === 401 || action.payload.silent)) {
                ...
                // Handle case when action.payload is undefined
                state.isLoading = false;
                state.error.status = true;
                state.error.message = "Network error or server is not responding"
                state.error.content = null
            }
        });
};
...
export const HRAsyncReducer = (builder, thunk) => {
    builder
        ...
        .addCase(thunk.fulfilled, (state, action) => {
            if (action.payload && action.payload.type == "signup") {
                state.isSignUp = true
                state.isLoading = false;
            ...
            if (action.payload && ((action.payload.type == "checkHR") || (action.payload.type == "HRLogin") || (action.payload.type == "HRforgotpassword"))) {
                state.isSignUp = true
                state.isLoading = false;
                state.isAuthenticated = true
                state.isAuthourized = true
                state.error.status = false;
                state.data = action.payload;
                // Clear logout flag when successfully logged in
                if (typeof window !== 'undefined') {
                    sessionStorage.removeItem('hasLoggedOut');
                }
            }
            if (action.payload && action.payload.type == "HRverifyemail") {
                state.isSignUp = true
                state.isLoading = false;
                state.isAuthenticated = true
                state.isAuthourized = true
            ...
            if (action.payload && action.payload.type == "HRcodeavailable") {
                state.isSignUp = true
                state.isLoading = false;
                state.isAuthenticated = true
            ...
            if (action.payload && action.payload.type == "HRResendVerifyEmail") {
                state.isSignUp = true
                state.isLoading = false;
                state.isAuthenticated = true
                state.isVerifiedEmailAvailable = true
            ...
            if (action.payload && (action.payload.message === "HR Logged Out Successfully" || action.payload.type == "HRLogout")) {
                // Reset all HR state to initial values on logout
        ...
        .addCase(thunk.rejected, (state, action) => {
            // Check if action.payload exists and has type property
            if (action.payload && action.payload.type == "signup") {
                state.isSignUp = false
                state.isLoading = false;
                state.error.status = true;
